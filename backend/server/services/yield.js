const soilTempYieldPredict = (cropData) => {
  const soiltempRatio = {};
  cropData.forEach((entry) => {
    const { Date, User_Temperature, Avg_Temperature } = entry;
    let Avg_Temperature_Weight = 0.0499;
    let userTemperatureWeight =
      Avg_Temperature_Weight * (User_Temperature / Avg_Temperature);
...
const fieldCapacityPredict = (cropData) => {
...
const soilMoistureYieldPredict = (cropData) => {
  ...
  cropData.forEach((entry) => {
    ...
    let userSoilMoistureWeight =
      Ideal_Soil_Moisture_Weight * (User_Soil_Moisture / Ideal_Soil_Moisture);
...
const soilRoughnessYieldPredict = (cropData) => {
...
const finalAnalyticalPredict = (cropData) => {
  const finalWeights = {
    Growth_Stage: -0.0082,
  ...
  let transformedData = cropData.map((row) => {
    ...
    const normalizeYield = Math.min((weightedSum + 100) / 2, 100);
    return {
      Date: row.Date,
...
const calculateFinalWeightedValue = (row, weights) => {
  ...
  const userTemperatureWeight =
    weights.Avg_Temperature * (row.User_Temperature / row.Avg_Temperature);
